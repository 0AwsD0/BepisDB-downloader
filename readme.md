...
## You can use simpler version of the program.
The simple program version is inside 'simple version' Dir in this repository.
It's "fire and forget" version that will download what you wanted and close it self afterwards.

### If you need to download cards in bulk - or cards from certain tags etc. - This app is for you
I'm creating this application to learn some python and have it as a backup means of batch card download. I have my own browser add-on that I wrote in JS that works just fine (not anymore it's bricked :p), but I'm working on this solution in case my broken add-on cease to work.(it did) Also, I will not publish source code of that add-on because it's made just for me - so it's a little bit broken but to the extent I don't bother to fix it. And that's how this projects begins. The plan is to use Selenium and do even more functionality than the current browser add-on can do. Add-on worked by clicking download buttons in order, waiting hard-coded  amount of time until download is finished (to prevent multiple downloads, since the site didn't like that), then clicking next button to move to the next page and repeated.

## How to use
- the software is in development, so it's advised to read code coments inside .py files to better understand it's inner workings //not really, it just works now ~Todd Howard
...
2. Select download type -> "basic"  allows to enter only "Name" and/or "Tag" to search -> "advanced" - allows to set up every search parameter available for selected game
3. Enter value for each prompted field or click "Enter" to get default value.
4. Wait for download to finish.
5. (Not mandatory) check below formula if you want to be sure every single card got downloaded. If not try finding which ones didn't.

## How it works
- The page for selected game is loaded and form is filled by the selenium. 
- After search button is auto clicked, program waits 5 seconds for website to load. 
...
- If the page had less than 23 cards on it, it's the last page (or only one) -> Download Finished
...
- You can do it by checking out program settings after it's start. (They are saved into config)
...
- to be sure that program downloaded all the cards you can use this formula: ((number of pages) - 1) * 23 + (number of cards on last page)
- for 7 pages, when last contains 4 cards it would loo like:  6*23+4 = 142 cards

- for less advanced users: do not use this script for downloading 'live content' -> if somebody uploads new cards into the site, and it falls into the conditions for your download, the script won't download them. That situation will change the cards order and can lead to omitting some cards in very active search or duplicate download. Imagine there are 30 cards on website - 1 page of 23 cards and 7 cards on page 2. You start downloading and in first seconds somebody uploads 3 more cards. The page can handle 23 cards, so last 3 cards are moved to page 2, where now lays 10 cards. But script doesn't know that - it's browser have a loaded page before that. It has the version of the website that does not have the cards moved. Script changes page to next one (page 2). You download those 10 cards  including 3 cards you already have.
